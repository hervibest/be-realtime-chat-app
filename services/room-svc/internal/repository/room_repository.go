...
type RoomRepository interface {
	...
	FindByUUIDAndUserID(ctx context.Context, db Querier, uuid string, userID string) (*entity.Room, error)
...
func (r *roomRepositoryImpl) FindByUUIDAndUserID(ctx context.Context, db Querier, uuid string, userID string) (*entity.Room, error) {
	room := new(entity.Room)
	query := `SELECT * FROM rooms WHERE uuid = $1 AND user_id = $2 AND deleted_at IS NULL`
	if err := pgxscan.Get(ctx, db, room, query, uuid, userID); err != nil {
		r.log.Error("failed to get query", zap.String("query", query), zap.Error(err))
		return nil, err
	}
	return room, nil
